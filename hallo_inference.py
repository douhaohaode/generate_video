...
def inference_process(args: argparse.Namespace,pose_weight,face_weight,lip_weight,inference_steps,cfg_scale,w,h):
    """
...
def hallo_inference_def(driving_audio, source_image,pose_weight,face_weight,lip_weight,inference_steps,cfg_scale,w,h):


    timestamp = int(time.time())
    output_path = "results/output_{}.mp4".format(timestamp)
    ...
    parser.add_argument("--source_image", type=str, required=False,
                        help="source image", default=source_image)
    parser.add_argument("--driving_audio", type=str, required=False,
                        help="driving audio", default=driving_audio)
    parser.add_argument(
        "--output", type=str, help="output video file name", default=output_path)
    parser.add_argument(
    ...
    inference_process(command_line_args,pose_weight,face_weight,lip_weight,inference_steps,cfg_scale,w,h)
