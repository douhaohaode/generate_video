...
def generate(audio_path, image_path, style_clip_path,
    pose_path, cfg_scale, max_gen_len, device):

    parser = argparse.ArgumentParser(description="inference for demo")
    parser.add_argument("--wav_path", type=str, default=audio_path, help="path for wav")
    parser.add_argument("--image_path", type=str, default=image_path, help="path for image")
    parser.add_argument("--disable_img_crop", dest="img_crop", action="store_false")
    parser.set_defaults(img_crop=True)

    parser.add_argument(
        "--style_clip_path", type=str, default=style_clip_path, help="path for style_clip_mat"
    )
    parser.add_argument("--pose_path", type=str, default=pose_path, help="path for pose")
    parser.add_argument(
        "--max_gen_len",
        type=int,
        default=max_gen_len,
    ...
    tmp_dir = f"dreamtalk/tmp/{args.output_name}"
    ...
    # wav2vec_model = (
    #     Wav2Vec2Model.from_pretrained("jonatasgrosman/whisper-large-zh-cv11")
    #     .eval()
    #     .to(device)
    # )
    ...
    with torch.no_grad():
        ...
        # get renderer
        renderer = get_netG("dreamtalk/checkpoints/renderer.pt", device)
        # render video
        output_video_path = f"dreamtalk/output_video/{args.output_name}.mp4"
        render_video(
            renderer,
            src_img_path,
            face_motion_path,
        ...
        # 添加水印
        # os.system(
        #     f'ffmpeg -y -i {no_watermark_video_path} -vf  "movie=dreamtalk/media/watermark.png,scale= 120: 36[watermask]; [in] [watermask] overlay=140:220 [out]" {output_video_path}'
        # )

        os.system(f'ffmpeg -y -i {no_watermark_video_path} {output_video_path}')
